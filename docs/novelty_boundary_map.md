...
The current toy does not provide that explicit evidence. Once the abstract baseline is tuned, a constant controller reaches 0.987 held-out success and beats the full-state controller. A future viable paper would need a task where tuned abstract baselines still fail because the compressed interface truly omits variables required for feasibility.
